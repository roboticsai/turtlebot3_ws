...
#include <neo_localization/GlobalPose.h>
...
public:
	NeoLocalizationNode()
	{
		...
		m_node_handle.param("update_min_d", update_min_d, 2.0);
		m_node_handle.param("update_min_a", update_min_a, -1.0);
		m_node_handle.param("max_d", max_d, 1.0);
		m_node_handle.param("max_a", max_a, -1.0);
		m_node_handle.param("max_update_count", max_update_count, 5);
		...
		service = m_node_handle.advertiseService("get_global_pose", &NeoLocalizationNode::get_global_pose, this);
...
protected:
	...
	bool get_global_pose(neo_localization::GlobalPose::Request  &req, neo_localization::GlobalPose::Response &res)
	{
		geometry_msgs::Pose global_pose;
		res.global_pose = global_pose;
		G_LOC = true;
		return true;
	}
	...
	void loc_update(const ros::TimerEvent& event)
	{
		...
		// Update the global pose if update_d or update_a is set by user
		bool update_d, update_a = false;
		if(update_min_d > 0.0) {
			double trans_dist = sqrt(pow(odom_pose[0]-prev_odom_pose[0],2) + pow(odom_pose[1]-prev_odom_pose[1],2));
			update_d = trans_dist >= update_min_d ? true : false; 
		}			
		if(update_min_a > 0.0) {
			double rotation = abs(prev_odom_pose[2] - odom_pose[2]);
			update_a = rotation >= update_min_a ? true : false;
		}
		if(update_d || update_a) {
			ROS_INFO("Updaing global pose!");	
			G_LOC = true;	
		}
		...
		for(int i = 0; i < m_sample_rate; ++i)
		{
			...
			if(m_solver.r_norm > best_score && G_LOC) {
		...
		if(mode > 0)
		{
			...
			// Disable analysing global pose from particle filter
			if(G_LOC) {
				double new_d = sqrt(pow(new_offset[0] - m_offset_x,2) + pow(new_offset[1] - m_offset_y,2));
				double new_a = new_offset[2] - m_offset_yaw;
				bool update_d = new_d < max_d ? true : false;
				bool update_a = new_a < max_a ? true : false;
				if(update_d && update_a) { update_count++; }
...
private:
	...
	double update_min_d = 0;				// Translational movement required before performing a filter update.
	double update_min_a = 0;				// Rotational movement required before performing a filter update.
	double max_d = 0;
	double max_a = 0;
	int update_count = 0;
	int max_update_count = 0;

	Matrix<double, 4, 4> base_to_odom_mat;
	Matrix<double, 3, 1> m_last_odom_pose;
	Matrix<double, 4, 4> m_grid_to_map;
	Matrix<double, 4, 4> m_world_to_map;
    Matrix<double, 3, 1> prev_odom_pose;
